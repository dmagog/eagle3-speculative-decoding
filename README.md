...
Воспроизведение [EAGLE-3](https://arxiv.org/abs/2503.01840) (Li, Wei, Zhang, Zhang — NeurIPS 2025) на [Qwen3-1.7B](https://www.kaggle.com/models/georgymamarin/qwen3-1-7b-eagle3-draft-head "Черновая голова Qwen3-1.7B на Kaggle") и [Qwen3-4B](https://www.kaggle.com/models/georgymamarin/qwen3-4b-eagle3-draft-head "Черновая голова Qwen3-4B на Kaggle"), на бесплатной Kaggle T4. Статья заявляет ускорение до 6.5×; я разобрал метод по частям и померил каждую — включая режимы, в которых он проигрывает обычной генерации.
...
### Домен решает больше, чем что-либо ещё

Одна и та же черновая голова даёт разброс от ускорения в два с половиной раза до замедления:
...
### Замедление на чужом домене — свойство глубины дерева, а не языка

Тот же срез по глубине, снятый на русскоязычном наборе, пересекает единицу:

| Глубина дерева | Ускорение | Длина принятия |
|---|---|---|
| 2 | 1.18× | 1.47 |
| 4 | 1.04× | 1.42 |
| 7 | 0.96× | 1.50 |
...
### Проверка целого дерева стоит примерно как генерация одного токена

Проход целевой модели не дорожает вплоть до 192 токенов — на этом метод и держится. Обычный шаг при этом вчетверо дороже теоретического минимума (43.5 против 10.8 мс), шина памяти загружена на четверть. Часть выигрыша метода — амортизация чужой неоптимальности, а не победа над памятью.

### На проверку уходит 81% цикла

На дереве `64/2/10` цикл раскладывается так: проверка 45.4 мс, черновики 9.0 мс, выбор пути 0.4 мс, остальное 1.4 мс — против 46.1 мс обычного шага. Ровно то, что предсказано выше: проверка стоит один обычный шаг, но приносит два с половиной токена вместо одного.

### При равной глубине ветвление даёт +39% к длине принятия

Цепочка набирает τ = 1.64, дерево той же глубины — τ = 2.28.

### Масштаб покупает длину принятия, но не скорость

Тот же протокол на Qwen3-4B: τ растёт на 9.2% (воспроизводимо на восьми прогонах), а ускорение сдвигается в пределах пары процентов и меняет знак от прогона к прогону. Черновая голова растёт вместе с целью: 137 млн параметров у пары 1.7B против 218 млн у 4B.

### Метод не портит выдачу

Проверок три. При T = 0 выдача совпадает потокенно, а редкие расхождения приходятся строго на позиции, где два верхних токена неразличимы по логиту. При T = 1 перестановочный тест не отвергает равенство распределений, причём с контролем на двух выборках обычной генерации. И прямое сравнение: распределение, из которого берётся первый токен, совпадает бит в бит.
...
tools/                  проверки, без которых числа в тексте разъезжаются
                        с расчётом, и сборка PDF
...
Пять инструментов: каждый появился после того, как очередной дефект дожил до публикации.
...
**`tools/make_pdf.sh`** собирает PDF: прячет служебный код, встраивает KaTeX и печатает через браузер. Появился после того, как в обоих опубликованных PDF формулы оказались сырым LaTeX — nbconvert грузит MathJax из сети, а браузер печатал страницу, не дождавшись его. Теперь сборка идёт офлайн и не пишет PDF вовсе, если отрисовалось не 18 формул или хоть одна дала ошибку.
...
Пересобрать PDF из выгруженных с Kaggle `.ipynb`:
...
<p align="right"><em>Сделал для курса ШАД в Сириусе, 2026</em></p>
